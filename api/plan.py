...
plan_model = plan_ns.model('plan', {
    'planname': fields.String(required=True, description='计划名称'),
    'description': fields.String(required=True, description='计划描述'),
    'begin_time': fields.DateTime(required=True, description='计划开始时间'),
    'deal_time': fields.DateTime(required=True, description='计划截止时间'),
    'finished': fields.Boolean(required=False,default=False,description='计划是否完成'),
})
delete_model = plan_ns.model('delete',{
    'id': fields.Integer(required=True, description='计划id')
})
update_model = plan_ns.model('update',{
    'id': fields.Integer(required=True, description='计划id'),
...
class Plan(Resource):
    ...
    def delete(self):
        ...
        try:
            ...
            return PlanService().delete(data.get('id'))
        except Exception as e:
            return Response.SEVER_ERROR(e)

    @plan_ns.expect(update_model)
    def put(self):
        """更新计划"""
        try:
            data = request.get_json()
            ...
            return PlanService().update(data)
        except Exception as e:
            return Response.SEVER_ERROR(e)
